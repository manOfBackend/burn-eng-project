...
import { getSentenceProblem, getUserInfo, submitWriting } from '@sayvoca/lib/api'
...
export default function WritingProblemView() {
  ...
  return (
    <WritingProblemForm isLoading={isLoading || isSuccess} level={user?.level ?? 1} problem={problem?.sentence ?? ''} onSubmit={onSubmit} />
  )
}
